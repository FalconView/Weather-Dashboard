  ...
  fetch(apiLocationUrl).then(function (response) {
    ...
    if (response.ok) {
      response.json().then(function (data) {
        ...
        if (data[0] == null) {
          window.alert(
            "The city name is invalid. Please enter a valid city name."
          );
          return;
...
var getWeatherData = function (lat, lon) {
  ...
  fetch(apiWeatherUrl).then(function (response) {
    ...
    if (response.ok) {
      ...
      window.alert(
        "Sorry. No data found for " + cityName + ". Try other city."
      );
    }
  });
};
...
var showCurrentWeather = function (data) {
  ...
  $(".today-weather > div > img")
    .attr(
      "src",
      "http://openweathermap.org/img/wn/" + weatherCond.icon + ".png"
    )
...
var showForecast = function (data) {
  ...
  for (var i = 0; i < 5; i++) {
    ...
    var dayContainerEl = $("<div>").addClass("day-forecast");
    ...
    var iconEl = $("<img>")
      ...
      .attr(
        "src",
        "http://openweathermap.org/img/wn/" + forecastData.icon + ".png"
      )
